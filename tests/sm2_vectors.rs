...
use libsmx::sm2::{
    get_e, get_z, sign_message, sign_with_k, verify, verify_message, PrivateKey, PublicKey,
    DEFAULT_ID,
};
...
fn test_sm2_gb_t_a1_different_ids() {
    ...
    for i in 0..signatures.len() {
        for j in i + 1..signatures.len() {
            assert_ne!(signatures[i], signatures[j], "不同用户标识应产生不同签名");
        }
    }
}
...
fn test_sm2_gb_t_a1_boundary() {
    ...
    let pri_key =
        PrivateKey::from_bytes(d_bytes.as_slice().try_into().unwrap()).expect("最小私钥应有效");
    let pub_key = pri_key.public_key();
...
fn test_sm2_verify_tampered_sig_fails() {
    ...
    assert!(
        verify(&e, &pub_key.as_bytes(), &sig).is_err(),
        "篡改签名后验签应失败"
    );
}
...
fn test_sm2_gb_t_a1_error_cases() {
    ...
    assert!(
        verify(&e, &wrong_pub_key.as_bytes(), &sig).is_err(),
        "错误公钥验签应失败"
    );
    ...
    assert!(
        verify(&wrong_e, &pub_key.as_bytes(), &sig).is_err(),
        "错误消息验签应失败"
    );
    ...
    assert!(
        verify(&e, &pub_key.as_bytes(), &wrong_sig).is_err(),
        "错误签名验签应失败"
    );
}
...
fn test_sm2_gb_t_a1_randomness() {
    ...
    for i in 0..signatures.len() {
        for j in i + 1..signatures.len() {
            assert_ne!(signatures[i], signatures[j], "随机生成的签名应不同");
        }
    }
}
