...
fn test_sm3_long_message() {
    ...
    assert_eq!(result1, result3, "Long message hash should be consistent with incremental update");
}
...
fn test_sm3_avalanche_effect() {
    ...
    // 计算不同位数（应该接近 50%）
    let diff_bits = hash1.iter().zip(hash2.iter())
    ...
    assert!(diff_ratio > 0.4 && diff_ratio < 0.6,
            "Avalanche effect weak: {}% bits differ", diff_ratio * 100.0);
}
